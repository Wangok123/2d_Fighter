...
namespace UnityCore.EventSystem
{
    public class UniEvent
    {
		...
		private static readonly Dictionary<int, LatLinkedList<Action<IEventMessage>>> Listeners = new Dictionary<int, LatLinkedList<Action<IEventMessage>>>(1000);
		...
		public static void Initialize()
		{
			...
			if (_isInitialize == false)
			{
				// 创建驱动器
				_isInitialize = true;
				GameDebug.Log($"{nameof(UniEvent)} initialize !");
			}
		}
		...
		public static void Destroy()
		{
			if (_isInitialize)
			{
				...
				_isInitialize = false;
				GameDebug.Log($"{nameof(UniEvent)} destroy all !");
			}
		}
		...
		public static void AddListener(int eventId, System.Action<IEventMessage> listener)
		{
			if (Listeners.ContainsKey(eventId) == false)
				Listeners.Add(eventId, new LatLinkedList<Action<IEventMessage>>());
		...
		public static void SendMessage(int eventId, IEventMessage message)
		{
			...
			LatLinkedList<Action<IEventMessage>> listeners = Listeners[eventId];
