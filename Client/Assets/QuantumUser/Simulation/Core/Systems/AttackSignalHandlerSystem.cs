...
{
    [Preserve]
    public unsafe class AttackSignalHandlerSystem : SystemSignalsOnly, 
        ISignalOnAttackHitboxActivate,
        ISignalOnAttackExecute,
        ISignalOnAttackHitTarget
    {
        ...
        public void OnAttackExecute(Frame frame, EntityRef attacker)
        {
            ...
            if (hits.Count > 0)
            {
                ...
                for (int i = 0; i < hits.Count; i++)
                {
                    ...
                    frame.Signals.OnAttackHitTarget(attacker, hit.Entity, transform->Position, hitPlayerTransform->Position);
                }
            }
        }

        public void OnAttackHitTarget(Frame frame, EntityRef attacker, EntityRef target, FPVector2 attackerPos, FPVector2 targetPos)
        {
