...
public class StateMachine
{
    private readonly Dictionary<string, Object> _blackboard = new Dictionary<string, Object>(100);
        ...
        public void AddNode(IStateNode stateNode)
        {
            ...
            {
                GameDebug.LogError($"State node already existed : {nodeName}");
            }
        }
        ...
        public void ChangeState(string nodeName)
        {
            ...
            if (node == null)
            {
                GameDebug.LogError($"Can not found state node : {nodeName}");
                return;
            }

            GameDebug.Log($"{_curNode.GetType().FullName} --> {node.GetType().FullName}");
            _preNode = _curNode;
            _curNode.OnExit();
            _curNode = node;
            _curNode.OnEnter();
        ...
        public Object GetBlackboardValue(string key)
        {
            ...
            {
                GameDebug.LogWarning($"Not found blackboard value : {key}");
                return null;
            }
        }
