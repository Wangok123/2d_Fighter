...
namespace LATNet
{
    public abstract class KCPSession
    {
        ...
        public void InitSession(uint sessionId, Action<byte[], IPEndPoint> udpSender, IPEndPoint remotePoint)
        {
            ...
            Handle.OutputAction = OnOutput;

            Handle.ReceiveAction = OnReceive;
        ...
        public void SendMsg(byte[] msg)
        {
            ...
            if (IsConnected)
            {
                byte[] buffer = KCPTool.Compress(msg);
        ...
        protected abstract void OnReceiveMsg(byte[] bytes);
        protected abstract void OnUpdate(DateTime now);

        public override bool Equals(object obj)
        {
            if (obj is KCPSession us)
            {
        ...
        {
            if (m_udpSender == null || m_remoteEndPoint == null)
            {
                KCPTool.Warn("Session Disconnected.Can not send msg.");
                return;
            }
            
            m_udpSender(buffer.ToArray(), m_remoteEndPoint); 
        }
