...
namespace LATNet
{
    public class KCPNet<T> where T : KCPSession, new()
    {
        ...
        private void OnClientSessionClosed(uint sessionId)
        {
            ...
            KCPTool.ColorLog(KCPLogColor.Green, $"Client Session Close, SessionID: {sessionId}");
        }
        ...
        public void SendMsg(uint sessionId, byte[] bytes)
        {
            if (sessionDic.TryGetValue(sessionId, out T session))
            {
                session.SendMsg(bytes);
            }
            else
            {
                KCPTool.Warn("Session Not Found, SessionID: " + sessionId);
            }
        }
